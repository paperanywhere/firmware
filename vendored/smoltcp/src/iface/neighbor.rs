...
impl Neighbor {
    /// The cached hardware address for this neighbor. Read-only accessor
    /// added by the paperanywhere fork so consumers can dump the
    /// neighbor cache for diagnostics without depending on smoltcp
    /// internals.
    pub fn hardware_addr(&self) -> HardwareAddress {
        self.hardware_addr
    }

    /// When this cache entry will expire. Useful for distinguishing
    /// "lookup-in-progress / hold-down" entries from fresh ones in a
    /// diag snapshot.
...
impl Cache {
    ...
    /// Iterate every (protocol_addr, neighbor) currently in the cache.
    /// Read-only — added by the paperanywhere fork for diag.
