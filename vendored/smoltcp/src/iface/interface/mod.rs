...
impl Interface {
    ...
    /// Read-only access to the neighbor cache (ARP table). Added by
    /// the paperanywhere fork so consumers can dump live neighbor
    /// entries for diagnostics without depending on smoltcp internals.
    /// Returns `None` on ip-medium interfaces that don't maintain a
    /// neighbor cache.
