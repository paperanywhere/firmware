...
// Cross-core patch (paperanywhere fork — see `vendored/embassy-net/`).
//
// Upstream embassy-net wraps the stack's `Inner` in a plain `RefCell`,
// which makes `Stack<'_>` neither `Send` nor `Sync` — it can't be
// accessed from a task running on a different executor / core.
//
// We sit it inside `embassy_sync::blocking_mutex::Mutex<CSRawMutex, _>`
// instead. That mutex's `CriticalSectionRawMutex` flavour is
// cross-core safe on chips with a multicore `critical_section` impl
// (esp-hal's is — it grabs the chip's shared CS spinlock). The
// outer Mutex is Sync, so `&Stack` is Send; that's the property the
// firmware needs to read `stack.local_ip()` from a task on a
// different core than the one running `net_task`.
//
// SAFETY: this works because every `with` / `with_mut` callback is
// non-async — the lock is acquired, the closure runs synchronously,
// the lock is dropped before any `.await`. Adding an `.await` inside
// a callback would deadlock.
...
// Storage for the stack's `Inner`. Pre-cross-core-fork this was a
// plain `RefCell<Inner>`; the fork wraps it in
// `BlockingMutex<CSRawMutex, _>` so `&Stack` is Sync.
pub struct StackResources<const SOCK: usize> {
    sockets: MaybeUninit<[SocketStorage<'static>; SOCK]>,
    inner: MaybeUninit<BlockingMutex<CriticalSectionRawMutex, RefCell<Inner>>>,
    #[cfg(feature = "dns")]
...
#[derive(Copy, Clone)]
pub struct Stack<'d> {
    inner: &'d BlockingMutex<CriticalSectionRawMutex, RefCell<Inner>>,
}

// SAFETY: the inner BlockingMutex<CSRawMutex, _> is Sync via the
// critical-section primitive, so &Stack can cross cores. We've kept
// every callback into the locked region synchronous, so a cross-core
// borrow can never overlap with another.
...
pub fn new<'d, D: Driver, const SOCK: usize>(
    ...
    let inner = &*resources
        .inner
        .write(BlockingMutex::new(RefCell::new(inner)));
...
impl<'d> Stack<'d> {
    fn with<R>(&self, f: impl FnOnce(&Inner) -> R) -> R {
        // Cross-core fork: acquire the outer mutex first (cheap CS
        // on esp-hal — one shared spinlock), then take the RefCell
        // shared borrow. The closure is sync, so the lock drops
        // before any caller-side .await.
